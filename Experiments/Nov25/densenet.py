...
def block(input, layers, in_features, growth, is_training, keep_prob):
  ...
  # feed following 4 layers
  # this_growth = 2
  # currentg = 6
  #
  # first = current
  # second = batch_activ_conv(current, features, currentg, 3, is_training, keep_prob)
  # current = tf.concat(3, (first, second))
  # features += currentg
  # currentg += this_growth
  # print features
  #
  # third = batch_activ_conv(current, features, currentg, 3, is_training, keep_prob)
  # current = tf.concat(3, (current, third))
  # features += currentg
  # currentg += this_growth
  # print features
  #
  # fourth = batch_activ_conv(current, features, currentg, 3, is_training, keep_prob)
  # current = tf.concat(3, (current, fourth))
  # features += currentg
  # currentg += this_growth
  # print features
  #
  # tmp = batch_activ_conv(current, features, currentg, 3, is_training, keep_prob)
  # first = second
  # second = third
  # third = fourth
  # fourth = tmp
  # current = tf.concat(3, (tmp, first))
  # current = tf.concat(3, (current, second))
  # current = tf.concat(3, (current, third))
  # features = 36
  # currentg += this_growth
  #
  # for idx in xrange(layers-5):
  #     tmp = batch_activ_conv(current, features, currentg, 3, is_training, keep_prob)
  #     first = second
  #     second = third
  #     third = fourth
  ...
  #     current = tf.concat(3, (current, second))
  #     current = tf.concat(3, (current, third))
  #     features += this_growth*4
  #     currentg += this_growth
  #     print features

  # for idx in xrange(layers-4):
  #     tmp = batch_activ_conv(current, features, growth, 3, is_training, keep_prob)
  #     first = second
  #     second = third
  #     third = fourth
...
def run_model(data, image_dim, label_count, depth):
  ...
  with tf.Session(graph=graph) as session:
    ...
    #   learning_rate = 0.1/(1+epoch*0.5)
      if epoch>=0 and epoch<=25: learning_rate = 0.1-0.0038*epoch
      if epoch>25 and epoch<=50: learning_rate = -0.1+0.004*epoch
      if epoch>50 and epoch<=75: learning_rate = 0.1-0.0038*(epoch-50)
      if epoch>75 and epoch<=100: learning_rate = -0.1+0.004*(epoch-50)
      if epoch>100 and epoch<=125: learning_rate = 0.1-0.0038*(epoch-100)
      if epoch>125 and epoch<=150: learning_rate = -0.1+0.004*(epoch-100)
      if epoch>150 and epoch<=175: learning_rate = 0.1-0.0038*(epoch-150)
      if epoch>175 and epoch<=200: learning_rate = -0.1+0.004*(epoch-150)
      if epoch>200 and epoch<=225: learning_rate = 0.1-0.0038*(epoch-200)
      if epoch>225 and epoch<=250: learning_rate = -0.1+0.004*(epoch-200)
      for batch_idx in xrange(batch_count):
